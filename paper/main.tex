...
\begin{itemize}
  ...
  \item \textbf{Open-source infrastructure}: 948 unit tests, CI/CD, PyPI distribution, Sphinx documentation, reproducible artifacts
\end{itemize}
...
\begin{table}[h]
  ...
  \begin{tabular}{lllll}
    ...
    \textbf{PyFeng} & Monte Carlo convergence (vanilla calls) & 5 paths & \textcolor{green}{\checkmark} & RMSE $< 2\%$ \\
    \midrule
    \textbf{Total} & & 19 & \textcolor{green}{\checkmark} & \\
    \bottomrule
  \end{tabular}
...
[Placeholder for results section with embedded figures]
...
\textbf{Figure source}: \texttt{scripts/figures/plot\_rila\_fia\_payoffs.py}

\subsection{Monte Carlo Convergence}

Figure~\ref{fig:mc_convergence} demonstrates convergence of MC pricing to analytical Black-Scholes as path count increases from 1000 to 100,000.

\textbf{Figure source}: \texttt{scripts/figures/plot\_mc\_convergence.py}
...
\textbf{Figure source}: \texttt{scripts/figures/plot\_bs\_parity.py}
...
\textbf{Figure source}: \texttt{scripts/figures/plot\_glwb\_fee\_surface.py}
...
\textbf{Figure source}: \texttt{scripts/figures/plot\_vm21\_cte\_sensitivity.py}
...
\subsection{Fixed Seeds}

All figures and Monte Carlo simulations use deterministic seeds (e.g., \texttt{seed=1234}) to ensure reproducibility. Seed values are documented in each script header.
...
The library includes 948 unit tests spanning core infrastructure, cross-validation, and regulatory prototypes, with continuous integration via GitHub Actions.
