...
\begin{abstract}
We present \texttt{annuity-pricing}, an open-source Python library for risk-neutral valuation of indexed annuity products (FIA, RILA, GLWB). The framework implements Black-Scholes and Monte Carlo pricing with integrated behavioral models (dynamic lapse, withdrawal rates) and SOA 2012 mortality tables, enabling competitive positioning analysis and regulatory capital estimation (VM-21 CTE$_{70}$, VM-22 reserves).

\textbf{Validation}: Black-Scholes pricing achieves $<0.01\%$ error vs.\ financepy; Heston stochastic volatility (COS method) matches QuantLib to $<1\%$; Monte Carlo converges to analytical benchmarks with RMSE $<0.5\%$ at 100k paths. The test suite comprises 2,703 tests (484 validation, 85\% coverage).

\textbf{Scope}: VM-21/VM-22 modules are \emph{prototypes} pending NAIC AG43 scenario compliance. A stress testing framework (historical, ORSA, reverse, sensitivity) is included for risk analysis. This work provides practitioners with transparent, reproducible tooling for annuity pricing research, addressing a gap in open-source actuarial software.
...
This paper introduces \texttt{annuity-pricing} (v1.0.0), a research-grade Python library addressing these requirements:
...
\begin{itemize}
  ...
  \item \textbf{Stress testing}: Historical crisis scenarios (7), ORSA standard tests, reverse stress testing, sensitivity analysis (tornado diagrams)
  \item \textbf{Open-source infrastructure}: 2,703 tests (85\% coverage), CI/CD, PyPI distribution, Sphinx documentation, reproducible artifacts
\end{itemize}
...
\begin{tabular}{ll}
\toprule
\textbf{Complete (Research-Grade)} & \textbf{Prototype (Pending NAIC)} \\
\midrule
Options: BS, Heston COS, SABR & VM-21/VM-22 scenarios \\
FIA/RILA payoffs \& fair solvers & AG43 prescribed paths \\
Behavioral: SOA lapse, withdrawal & Regulatory filing format \\
GLWB path simulation & \\
...
All results are reproducible via \texttt{make reproduce} from the repository root. Fixed random seeds (documented in script headers) ensure deterministic Monte Carlo output. The library is archived on Zenodo under tag \texttt{v1.0.0-paper} with the current commit hash available via \texttt{git rev-parse HEAD}. See Section~\ref{sec:reproducibility} for full reproduction instructions.
...
\begin{table}[h]
  ...
  \begin{tabular}{llrrl}
    \toprule
    \textbf{Category} & \textbf{Validation} & \textbf{Tests} & \textbf{Error} & \textbf{Status} \\
    \midrule
    \multicolumn{5}{l}{\textit{Cross-Library Validation}} \\
    Black-Scholes & vs.\ financepy & 15 & $<0.01\%$ & \textcolor{green}{PASS} \\
    Heston COS & vs.\ QuantLib & 12 & $<1\%$ & \textcolor{green}{PASS} \\
    SABR & vs.\ QuantLib & 8 & 0\% & \textcolor{green}{PASS} \\
    Greeks & vs.\ financepy & 20 & $<0.25\%$ & \textcolor{green}{PASS} \\
    Yield Curves & vs.\ QuantLib & 5 & $<0.1$ bps & \textcolor{green}{PASS} \\
    \midrule
    \multicolumn{5}{l}{\textit{Anti-Pattern Tests (Bug Prevention)}} \\
    Arbitrage Bounds & Option $\leq$ Underlying & 8 & --- & \textcolor{green}{PASS} \\
    Put-Call Parity & $|C - P - S e^{-qT} + Ke^{-rT}| < \epsilon$ & 12 & $<10^{-10}$ & \textcolor{green}{PASS} \\
    Floor Enforcement & FIA credit $\geq 0$ & 6 & --- & \textcolor{green}{PASS} \\
    Buffer Mechanics & Buffer absorbs first X\% & 10 & --- & \textcolor{green}{PASS} \\
    \midrule
    \multicolumn{5}{l}{\textit{Monte Carlo}} \\
    Convergence & MC $\to$ BS at 100k paths & 25 & $<0.5\%$ & \textcolor{green}{PASS} \\
    Antithetic Variance & $\sigma^2_{\text{anti}} < \sigma^2_{\text{std}}$ & 15 & --- & \textcolor{green}{PASS} \\
    \midrule
    \textbf{Total} & & \textbf{484} & & \\
    \bottomrule
  \end{tabular}
  \caption{Validation test summary. All 484 validation tests pass. Anti-pattern tests prevent known pricing bugs.}
...
This section provides concrete numeric examples demonstrating the library's pricing engines.

\subsection{Example 1: FIA Fair Cap Solver}

\textbf{Problem}: Given an option budget of 3\% of account value, what cap rate can an insurer offer on a 1-year S\&P 500 FIA with 100\% participation?

\textbf{Inputs}:
\begin{itemize}
  \item Spot: $S_0 = 4500$
  \item Risk-free rate: $r = 4.5\%$
  \item Dividend yield: $q = 1.5\%$
  \item Volatility: $\sigma = 18\%$
  \item Option budget: 3\% $\times$ \$100,000 = \$3,000
\end{itemize}

\textbf{Solution}: The FIA payoff is a capped call: $\min(\max(R, 0), c)$. This replicates as:
\begin{equation}
  \text{Capped Call} = \text{Call}(K=S_0) - \text{Call}(K=S_0(1+c))
\end{equation}

Solving for $c$ such that the call spread equals the budget:
\begin{align}
  C(S_0, S_0) - C(S_0, S_0(1+c)) &= 0.03 \times S_0 \\
  C(4500, 4500) - C(4500, 4500(1+c)) &= 135
\end{align}

Using Black-Scholes: $C(4500, 4500) = \$339.12$. Binary search yields $c = 8.2\%$.

\textbf{API call}:
\begin{lstlisting}
from annuity_pricing import FIAPricer, MarketParams
market = MarketParams(spot=4500, risk_free_rate=0.045,
                      dividend_yield=0.015, volatility=0.18)
pricer = FIAPricer(market_params=market)
result = pricer.solve_fair_cap(option_budget=0.03, term=1.0)
# result.fair_cap = 0.082 (8.2%)
\end{lstlisting}

\subsection{Example 2: RILA Buffer Payoff}

\textbf{Problem}: Calculate the credited return for a 10\% buffer RILA when the index returns $-15\%$.

\textbf{Inputs}:
\begin{itemize}
  \item Index return: $R = -15\%$
  \item Buffer: $b = 10\%$
  \item Cap: $c = 12\%$ (not relevant for losses)
\end{itemize}

\textbf{Solution}: The buffer absorbs the first 10\% of losses:
\begin{align}
  R_{\text{credit}} &= \max(R + b, -\infty) \quad \text{(loss beyond buffer)} \\
  &= \max(-0.15 + 0.10, -\infty) \\
  &= -0.05 \quad \textbf{(client loses 5\%)}
\end{align}

\textbf{Interpretation}: The 10\% buffer absorbed 10 percentage points. The remaining 5\% loss passes to the client. If the return had been $-8\%$, the buffer would fully absorb it (credited return = 0\%).

\subsection{Example 3: GLWB Fair Fee Estimation}

\textbf{Problem}: Estimate the fair annual fee for a GLWB rider with 5\% compound rollup and 5\% lifetime withdrawal rate.

\textbf{Inputs}:
\begin{itemize}
  \item Initial account value: \$100,000
  \item Age at issue: 60
  \item Rollup rate: 5\% compound (until first withdrawal)
  \item Withdrawal rate: 5\% of GWB annually
  \item Mortality: SOA 2012 IAM
  \item Investment return: 6\% (expected)
  \item Volatility: 18\%
\end{itemize}

\textbf{Solution}: The fair fee $f$ satisfies:
\begin{equation}
  \E\left[\text{PV(Guaranteed Payments)}\right] = \E\left[\text{PV(Fee Revenue)}\right]
\end{equation}

Monte Carlo simulation (10,000 paths) with mortality-weighted payoffs yields:
\begin{itemize}
  \item PV of guaranteed payments: \$18,450
  \item Required fee revenue (at $f = 1.25\%$): \$18,520
\end{itemize}

\textbf{Result}: Fair fee $\approx 1.25\%$ annually on account value.

\subsection{Example 4: VM-21 CTE$_{70}$ Reserve (Prototype)}

\textbf{Problem}: Calculate the CTE$_{70}$ reserve for a GLWB block under simplified scenarios.

\textbf{Inputs}:
\begin{itemize}
  \item Account value: \$1,000,000
  \item Guaranteed base: \$1,200,000 (20\% in-the-money)
  \item Withdrawal rate: 5\%
  \item Age: 65
  \item Scenarios: 1,000 simplified paths (not NAIC AG43)
\end{itemize}

\textbf{Solution}: For each scenario $i$, compute the present value of shortfall (when account depletes before death):
\begin{equation}
  \text{Shortfall}_i = \max\left(0, \text{PV(Guaranteed)} - \text{PV(Account)}\right)
\end{equation}

Sort scenarios by shortfall (descending). CTE$_{70}$ = mean of worst 30\%:
\begin{equation}
  \text{CTE}_{70} = \frac{1}{300}\sum_{i=1}^{300} \text{Shortfall}_{(i)}
\end{equation}

\textbf{Result}: CTE$_{70}$ = \$145,000 (14.5\% of account value).

\textbf{Disclaimer}: This uses simplified scenarios. Production VM-21 requires NAIC AG43 prescribed paths.

\subsection{Example 5: Stress Test Reserve Impact}

\textbf{Problem}: Estimate reserve impact under a GFC-like stress scenario.

\textbf{Inputs} (GFC 2008 calibration):
\begin{itemize}
  \item Equity shock: $-50\%$
  \item Rate shock: $-200$ bps
  \item Volatility spike: $+25\%$ (absolute)
  \item Lapse shock: $+50\%$ (relative)
  \item Base reserve: \$100,000
\end{itemize}

\textbf{Solution}: Using sensitivity coefficients:
\begin{align}
  \Delta R &= R_0 \times (\alpha_{\text{eq}} \cdot \Delta S + \alpha_r \cdot \Delta r + \alpha_\sigma \cdot \Delta\sigma) \\
  &= 100{,}000 \times (-0.8 \times -0.50 + (-10) \times -0.02 + 0.15 \times 0.25) \\
  &= 100{,}000 \times (0.40 + 0.20 + 0.0375) \\
  &= \$63{,}750
\end{align}

\textbf{Result}: Reserve increases by \$63,750 (+64\%) under GFC stress, requiring \$163,750 total.

% ============================================================================
% Section 7: Regulatory Prototype (VM-21 and VM-22)
% ============================================================================
...
The library includes a comprehensive stress testing framework for risk analysis.

\subsection{Methodology}

Four stress testing approaches are implemented:

\begin{enumerate}
  \item \textbf{Historical Crisis Scenarios}: Seven historical crises (Black Monday 1987, LTCM 1998, Dot-com 2000, GFC 2008, COVID 2020, etc.) with calibrated equity, rate, and volatility shocks
  \item \textbf{ORSA Standard Scenarios}: Regulatory-style scenarios including base, moderately adverse, and severely adverse
  \item \textbf{Reverse Stress Testing}: Bisection search to find parameter values causing reserve thresholds to breach (e.g., reserve exhaustion, RBC $<200\%$)
  \item \textbf{Sensitivity Analysis}: One-at-a-time (OAT) parameter sweeps generating tornado diagrams ranked by impact
\end{enumerate}

\subsection{Stress Impact Model}

Reserve impact is estimated via a simplified sensitivity model:
\begin{align}
  \Delta R &= R_0 \times (\alpha_{\text{eq}} \cdot \Delta S + \alpha_r \cdot \Delta r + \alpha_\sigma \cdot \Delta\sigma + \alpha_{\text{lapse}} \cdot \Delta\lambda)
\end{align}
where $\alpha$ coefficients represent reserve sensitivities to equity ($\alpha_{\text{eq}} \approx -0.8$), rates ($\alpha_r \approx -10$), volatility ($\alpha_\sigma \approx 0.15$), and lapse ($\alpha_{\text{lapse}} \approx -0.10$).
...
\begin{table}[h]
  ...
  \begin{tabular}{llrrr}
    \toprule
    \textbf{Scenario} & \textbf{Type} & \textbf{Equity} & \textbf{Rate} & \textbf{Reserve $\Delta$} \\
    \midrule
    Base Case & ORSA & 0\% & 0 bps & 0\% \\
    Moderately Adverse & ORSA & $-20\%$ & $-50$ bps & $+21\%$ \\
    Severely Adverse & ORSA & $-40\%$ & $-100$ bps & $+42\%$ \\
    GFC 2008 & Historical & $-50\%$ & $-200$ bps & $+60\%$ \\
    COVID 2020 & Historical & $-34\%$ & $-150$ bps & $+42\%$ \\
    \bottomrule
  \end{tabular}
  \caption{Stress test results showing reserve impact across ORSA and historical scenarios. Base reserve: \$100,000. Reserve increases indicate higher capital requirements under stress.}
  \label{tab:stress}
\end{table}

% ============================================================================
% Section 8: Results and Figures
% ============================================================================
...
The library is specified via \texttt{pyproject.toml} (v1.0.0) with:
\begin{itemize}
  \item Python 3.10--3.13
  \item Core dependencies: numpy, scipy, pandas, pyarrow
...
Artifacts are archived on Zenodo under tag \texttt{v1.0.0-paper} with DOI to be assigned upon publication. The archive includes:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{PyPI}: \url{https://pypi.org/project/annuity-pricing/} (v1.0.0)
...
The library includes 2,703 unit tests spanning core infrastructure, cross-validation, stress testing, and regulatory prototypes, with continuous integration via GitHub Actions.
